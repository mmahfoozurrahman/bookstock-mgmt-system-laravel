    ...
    <div class="flex-1 p-6 lg:p-8">
        ...
        <div class="bg-white rounded-xl shadow overflow-hidden">
            ...
            <div class="overflow-x-auto">
                <div class="p-6">
                    <form class="p-6 space-y-6" method="POST" action="{{ route('authors.update', $author->id) }}">
                        @csrf
                        @method('PUT')
                        <div>
                        ...
                        <div>
                            ...
                            <input type="email" id="email" name="email" required
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
                                placeholder="Enter author email" value="{{ $author->email }}" />
                            @error('email')
                                <p class="text-red-500 text-sm mt-1">
                                    {{ $message }}
                                </p>
                        ...
                        <div>
                            ...
                            <textarea id="bio" name="bio" rows="4"
                                ...
                                placeholder="Enter author biography">{{ $author->bio }}</textarea>
                            @error('bio')
                                <p class="text-red-500 text-sm mt-1">
                                    {{ $message }}
                                </p>
                        ...
                        <div>
                            ...
                            <select id="status" name="status"
                                ...
                                <option value="inactive" {{ $author->status == 'inactive' ? 'selected' : '' }}>Inactive
                                </option>
                            </select>
                            @error('status')
                                <p class="text-red-500 text-sm mt-1">
